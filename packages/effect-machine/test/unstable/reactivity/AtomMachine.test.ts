...
const mount = <A>(registry: AtomRegistry.AtomRegistry, atom: Atom.Atom<A>) =>
  Effect.acquireRelease(Effect.sync(() => registry.mount(atom)), (release) => Effect.sync(release))
...
const makeInputCounterMachine = () => {
  const InputCounterStates = Machine.state({
...
describe("AtomMachine", () => {
  ...
  it.effect("reactively exposes an invoked child machine", () =>
    Effect.scoped(Effect.gen(function*() {
      ...
      const updated = yield* waitForResult(registry, childAtoms.result, (state) =>
        Option.isSome(state) && state.value.state.value.value === 2)
      assert(Option.isSome(updated))
      const selectedUpdated = yield* waitForResult(registry, selectedCount, (state) =>
        Option.isSome(state) && state.value.value === 2)
      assert(Option.isSome(selectedUpdated))
      assert.strictEqual(selectedUpdated.value.value, 2)
      const selectedUpdatedSnapshot = yield* waitForResult(registry, selectedCountSnapshot, (state) =>
        Option.isSome(state) && state.value.value.value === 2)
      assert(Option.isSome(selectedUpdatedSnapshot))
      assert.strictEqual(selectedUpdatedSnapshot.value.value.value, 2)
      yield* Effect.sync(() =>
        registry.set(parentAtoms.send, new ReadValue({}))
      )
  ...
  it.effect("reactively exposes a dynamically spawned child by family and runtime id", () =>
    Effect.scoped(Effect.gen(function*() {
      ...
      const root5 = Machine.state({ states: { Count } })
      const parent = Machine.make({
        effects: {
          source1: ({ children }: Machine.Machine.InvokeContext<
            {
              readonly "": {
                readonly initial: "Count"
                readonly states: {
  ...
  it.effect("creates retained atom families for dynamically spawned children", () =>
    Effect.scoped(Effect.gen(function*() {
      ...
      const childMachine = makeCounterMachine()
      const Child = Machine.childFamily(childMachine)
      const root6 = Machine.state({ states: { Count } })
      const parent = Machine.make({
        effects: {
          source1: ({ children }: Machine.Machine.InvokeContext<
            {
      ...
      const updated = yield* waitForResult(registry, count, (value) => Option.isSome(value) && value.value.value === 3)
      assert(Option.isSome(updated))
  ...
  it("uses Effect key equality without retaining family values permanently", async () => {
    class FamilyKey extends Data.Class<{
      readonly id: string
    }> {
    }
  ...
  it.effect("retains one keyed machine owner through every public projection", () =>
    Effect.scoped(Effect.gen(function*() {
      ...
      const stopped = yield* Machine.watch(ref).pipe(Stream.runCollect, Effect.forkScoped)
  ...
  it.effect("projects static and reactive event acceptance across runtime lifecycles", () =>
    Effect.scoped(Effect.gen(function*() {
      ...
      assert.strictEqual(yield* waitForResult(registry, stoppedCanComplete, (accepted) => !accepted), false)
    })))
    ...
    Effect.scoped(Effect.gen(function*() {
      ...
      const machine = Machine.make({
        ...
        states: {
          Ready: {
            initial: {
              editor: { decoded: true, data: new Editor({}) },
              network: { decoded: true, data: new Network({}) }
            },
            states: {
              editor: {
                initial: {
  ...
  it.effect("stops the machine when the registry is disposed", () =>
    Effect.gen(function*() {
      ...
      const watcher = yield* Machine.watch(ref).pipe(Stream.runCollect, Effect.forkScoped)
      yield* Effect.sync(() => registry.dispose())
