...
describe("machine protocols", () => {
  it("rejects forged, misclassified, and overlapping event descriptors", () => {
    const states = Machine.state({ states: { ProtocolIdle } })
    assert.throws(() => Machine.make({ root: states, events: [PublicEvent] as any }), /expected an event protocol/)
    assert.throws(
  ...
  it.effect("keeps the complete event protocol private across handler clones", () =>
    Effect.gen(function*() {
      ...
      assert.deepStrictEqual(
        ...
        >(machine, { _tag: "PublicEvent", value: "public" }),
      ...
      assert.deepStrictEqual(
        ...
        >(machine, { _tag: "InternalEvent", value: "internal" }),
