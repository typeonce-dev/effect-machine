...
const generatedResumeCases = generatedCases.map((generated) => {
  ...
  for (let index = 0; index < reference.steps.length; index++) {
    if (reference.steps[index]!.microsteps.some(({ changed }) => changed)) {
      lastObservable = index
    }
  }
...
const canonicalLogicalSnapshot = (value: unknown): unknown => {
  if (Array.isArray(value)) {
    return value.map(canonicalLogicalSnapshot)
  }
...
const assertLogicalSnapshotEquivalent = Effect.fn(
  function*(machine: any, expected: unknown, actual: unknown, label: string) {
...
const deliverAndObserve = Effect.fn(function*(machine: any, ref: Machine.MachineRef<any, any, any, any>, event: {
  ...
  const observed = yield* waitForSnapshot(ref, (snapshot) =>
    snapshot.status === "error" ||
    (isDeepStrictEqual(snapshot.state, planned.next) && (!planned.done || snapshot.status === "done"))).pipe(
...
describe("machine operation totality", () => {
  it.effect.prop("never defects while planning or round-tripping reachable snapshots from valid finite models", {
