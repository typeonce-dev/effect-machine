...
describe("Machine inspection", () => {
  ...
  const machine = Machine.make({
    ...
    initialConfiguration: (root) =>
      root.resolve(({ target }) =>
        target.from((to) => to.root.decoded(new Root({}), (root) => root.idle.decoded(new Idle({}))))
      )
  }).handle({
    states: {
      root: {
  ...
  it("keeps choice initial paths while configuration remains active-only", () => {
    ...
    const choiceMachine = Machine.make({
      ...
      initialConfiguration: (root) =>
        root.resolve(({ target }) => (target.from((to) => to.Flow.decoded(new Root({}), (flow) => flow.Routing()))))
    })
  ...
  it("requires statically selected transitions", () => {
    ...
    expect(flat.handle).type.not.toBeCallableWith({ states: { idle: { on: { Reset: () => undefined } } } })
